...
# ]
# ///
"""Isolated Toponymy topic-modelling worker for raudio (`raudio feature topics`).
...
raudio project (uv reads the PEP 723 block above, builds a sealed env, caches
...
def _require(schema_names: list[str], db: Path) -> None:
    ...
    if missing:
        sys.exit(
            ...
            "Run `raudio feature text_embedding` and `raudio feature atlas --space text` first."
        )
