...
- **Re-run gates** (no servers needed): `P2.1a/P2.1b` (cross-import both directions), `P2.3` (backend corpus-literals), `P2.8` (no raudio/rmedia imports in backend/), `P3.2` (frontend corpus-literals) all print **GATE OK**.
- **P4.2** — `docs/REPRODUCE.md` gained the two-execution-paths note (legacy per-column vs the `rmedia pipeline` Ray driver); the Makefile gained `pipeline-plan` / `pipeline-run STAGE=…` / `pipeline-index` / `features-all-ray`. `uv run rmedia pipeline plan` prints the 8-stage DAG (text_embedding, summary, frame_embedding, caption, caption_embedding, extract_frames, diarize, voiceprint) with shape/table/gate/client/actors×GPU.
