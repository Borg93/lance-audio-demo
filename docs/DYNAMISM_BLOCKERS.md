...
| 15 | **Fixed filter fields** `language/namn/referenskod/extraid/topic` in spec + builder | `search/spec.py:43-46`; `search/filters.py:15-46` | new facet filterable without 3 edits | build filters from `facet`-role columns |
...
| 25 | **Karaoke render tied to `alignments_json`** | `player-pane.svelte:46-72` | non-speech corpus (blank transcript) | role `alignments,modality=audio` → conditional |
