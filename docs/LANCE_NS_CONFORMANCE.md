...
| `lancekit` catalog transports (`MEDIA_READ/WRITE_BACKEND=catalog`) | catalog `POST /v1/table/{id}/{query,merge_insert,delete}` + `describe` | **PROVEN LIVE 2026-07-23** against the catalog booted from the lance-ns checkout over RustFS: schema round-trip, merge saves, delete, the 409 handshake on their version primitive (`describe?load_detailed_metadata=true`), replace-protects-humans, and 6 captured spec-2-0-2 RunEvents from their inline emitter (our `lineage_emit` no-ops via `effective_lineage_sink`). Wire notes fixed on OUR side: merge bodies must be Arrow-IPC **stream** (IPC file → their Rust reader errors); reads return IPC **file**. Evidence: `tests/test_catalog_live.py`. |
| The annotator SERVICE in catalog mode | the same catalog endpoints, driven by the real product | **PROVEN LIVE 2026-07-24 (milestone 2)**: the FastAPI app in `MEDIA_READ/WRITE_BACKEND=catalog` — version header + 409 + carry-forward reads all sourced from the catalog (`TableReader.table_version()`), browser E2E green in that mode (19+18), full 51 green back in direct default. Catalog table id settings-derived; E2E seeding through `create?mode=overwrite`. Version-HISTORY surface stays direct pre-merge (scope note in the handoff). Evidence: `tests/test_annotator_catalog_live.py`. |
