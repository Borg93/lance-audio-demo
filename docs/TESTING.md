...
(`src/raudio/{model,ingest,media,asr,retrieval}/…`, `backend/app.py`). Every
...
|---|---|---|
| **vLLM HTTP** | `VLLMEmbeddingClient` (`embed_text`/`embed_image`, `retrieval/embeddings.py`), `VLLMReranker.rerank` + `QwenVLReranker` (`retrieval/reranker.py`) | Mocking the HTTP transport just restates our own request shape. The smoke test already covers the 503-when-down contract at integration altitude. |
...
| 13 | `retrieval/embeddings.py::_square_crop_resize` + `_image_to_data_url` | **Medium** | unit (Pillow CPU) | `_square_crop_resize`: non-square 640×480 → output `(_IMAGE_SIDE, _IMAGE_SIDE)`; **assert against the `_IMAGE_SIDE` constant** (`embeddings.py:134`) so the documented 448→392 vision-token fix flips one expectation. `_image_to_data_url`: PIL and raw bytes both → `data:image/jpeg;base64,` decodable JPEG; RGBA → `convert('RGB')` no raise; int/str → `TypeError`. |
...
| `VLLMEmbeddingClient` / `VLLMReranker` / `QwenVLReranker` (`retrieval/embeddings.py`, `retrieval/reranker.py`) | HTTP/threadpool plumbing against vLLM; the testable pure helpers (`l2_normalize`, `image_to_data_url`) are extracted to `retrieval/utils.py`. |
