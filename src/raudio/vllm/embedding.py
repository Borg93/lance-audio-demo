...
# A single OpenAI-style chat turn: ``{"role": ..., "content": [...]}``. The
# values are heterogeneous (str / nested lists), so `object`, not a deep generic.
...
class VLLMEmbeddingClient:
    ...
    def embed_text(self, texts: list[str]) -> np.ndarray:
        ...
        return l2_normalize(self._t.map(self._embed_one, messages, concurrency=self.text_concurrency))
    ...
    def embed_image(self, images: list[bytes]) -> np.ndarray:
        ...
        return l2_normalize(self._t.map(self._embed_one, messages, concurrency=self.image_concurrency))

    def _embed_one(self, messages: list[_ChatMessage]) -> list[float]:
        body = {
            "model": self.model,
            "messages": messages,
        ...
        return self._t.post("/v1/embeddings", body)["data"][0]["embedding"]

    def _text_message(self, text: str) -> list[_ChatMessage]:
        return [
    ...
    def _image_message(self, image: bytes) -> list[_ChatMessage]:
        return [
            {"role": "system", "content": [{"type": "text", "text": self.instruction}]},
            {
                "role": "user",
