...
The drivers in :mod:`rmedia.core.driver` take a zero-arg ``factory`` that each
...
from rmedia.features.embed_columns import VECTOR_TYPE, _vectors_to_arrow
...
def summary_compute(url: str) -> Callable[[pa.Table], pa.Array]:
    from rmedia.clients.summarize import VLLMSummarizeClient
...
def caption_compute(url: str, model: str, instruction: str | None) -> Callable[[list[bytes]], pa.Array]:
    from rmedia.clients.caption import CAPTION_INSTRUCTION, VLLMCaptionClient
