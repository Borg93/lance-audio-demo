"""``rmedia pipeline`` — plan and run registry stages on Ray Data actor pools."""
...
def cmd_run(
    ...
    from rmedia.core import driver
    from rmedia.features.ray_bindings import bind_column_stage

    cli: CliContext = ctx.obj
    stage = STAGES.get(stage_name)
    if stage is None:
        raise typer.BadParameter(f"unknown stage {stage_name!r} — see `rmedia pipeline plan`")
...
def cmd_index(
    ...
    from rmedia.features.indexing import plan_indexes, run_indexes
