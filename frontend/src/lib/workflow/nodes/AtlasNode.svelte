...
{#if cfg && rt}
  <NodeShell {id} title="Atlas selection" status={rt.status} {selected}>
    <div class="flex flex-col gap-2">
      ...
      <button
        type="button"
        class="nodrag inline-flex items-center justify-center gap-1.5 rounded border border-border bg-background px-2 py-1 text-[11px] font-medium text-foreground transition-colors hover:bg-muted"
        onclick={(e) => {
          e.stopPropagation();
          openModal();
        }}
