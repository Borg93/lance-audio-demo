...
<div
  class="flex max-w-[8.5rem] flex-col gap-1 rounded-lg border border-border bg-card/95 p-1.5 shadow-md backdrop-blur"
>
  <span class="px-1 text-[10px] font-medium tracking-wide text-muted-foreground uppercase">
    Drag to add
  </span>
  {#each NODE_KINDS as kind (kind)}
    <button
      ...
      class="flex cursor-grab items-center gap-1.5 rounded border border-border bg-background px-2 py-1 text-[11px] text-foreground transition-colors hover:bg-muted active:cursor-grabbing"
    >
