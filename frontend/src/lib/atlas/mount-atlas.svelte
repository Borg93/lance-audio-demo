      ...
      <div class="flex h-full min-h-0 flex-col border-t border-border bg-card/30">
        <div class="flex items-center gap-2 border-b border-border px-3 py-1.5 text-xs">
          ...
          <div class="relative ml-auto">
            <button
              type="button"
              class="flex items-center gap-1 rounded px-2 py-0.5 text-muted-foreground hover:bg-secondary/60 hover:text-foreground"
            ...
            {#if showColsPicker}
              ...
              ></button>
              <div
                class="absolute right-0 top-full z-20 mt-1 max-h-72 w-48 overflow-y-auto rounded-md border border-border bg-card p-1 shadow-md"
              >
                {#each OFFERED_COLS as c (c.key)}
                  {@const on = visibleCols.includes(c.key)}
                  <button
                    type="button"
                    class="flex w-full items-center gap-2 rounded px-2 py-1 text-left hover:bg-secondary/50"
                  ...
                  >
                    ...
                    >
                      {#if on}<Check class="size-2.5 text-primary-foreground" />{/if}
                    </span>
