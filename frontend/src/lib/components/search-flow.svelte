...
<div class="flex flex-col gap-3 text-[11px]">
  ...
  <div class="flex flex-col gap-1.5">
    ...
    <div class="grid grid-cols-3 gap-2">
      ...
      <div class="rounded border border-border border-l-2 border-l-primary bg-surface2 p-2">
        ...
        <div class="text-[10px] text-muted-foreground">what a clip is about, even in other words (text vector)</div>
      </div>
  ...
  <div class="flex flex-col gap-1.5">
    ...
    <div class="overflow-hidden rounded border border-border">
      <!-- header -->
      <div class="grid grid-cols-[64px_1fr_1fr] bg-surface2 text-[10px] font-medium text-foreground">
      ...
      <div class="grid grid-cols-[64px_1fr_1fr] text-[10px] text-muted-foreground">
        ...
        <div class="border-b border-l border-border p-1.5">transcript text (BM25). Good at: exact terms, names.</div>
      </div>
      <!-- meaning -->
      <div class="grid grid-cols-[64px_1fr_1fr] text-[10px] text-muted-foreground">
        ...
        <div class="border-b border-l border-border p-1.5"><code>text_embedding</code> of each transcript. Good at: topics, paraphrases.</div>
      </div>
      <!-- image -->
      <div class="grid grid-cols-[64px_1fr_1fr] text-[10px] text-muted-foreground">
  ...
  <div class="flex flex-col gap-1.5">
    ...
    <div class="grid grid-cols-3 gap-2">
      <div class="rounded border border-border bg-background px-2 py-1.5 text-center">
        FTS leg<div class="text-[10px] text-muted-foreground">BM25 on text</div>
      </div>
      <div class="rounded border border-border bg-background px-2 py-1.5 text-center">
        Text-vector leg<div class="text-[10px] text-muted-foreground"><code>text_embedding</code></div>
      </div>
      <div class="rounded border border-border bg-background px-2 py-1.5 text-center">
  ...
  <!-- 5) RRF, CONCRETELY -->
  <div class="flex flex-col gap-1.5 rounded-md border border-dashed border-border bg-background p-2">
    ...
    <div class="rounded border border-border bg-surface2 p-1.5 font-mono text-[10px] text-foreground">
      A = 1/62 + 1/61 ≈ 0.0326&ensp;<span class="rounded bg-highlight/30 px-1">in both → wins</span><br />
      C = 1/61 + 1/62 ≈ 0.0326&ensp;<span class="rounded bg-highlight/30 px-1">in both → wins</span><br />
  ...
  <div class="flex flex-col gap-1.5">
    ...
    <p class="text-muted-foreground">
      The Balance slider is a <strong class="text-foreground">2-way blend</strong> of the actual scores; it only
      exists for Hybrid (keyword ↔ meaning). It cannot describe 3 legs, so the moment you add an image, fusion
      falls back to equal-weight RRF and the slider is <strong class="text-foreground">ignored</strong>.
    </p>
    <div class="grid grid-cols-2 gap-2 text-[10px]">
  ...
  <div class="flex flex-col gap-1.5 rounded-md border border-border bg-card p-2">
    ...
    <ul class="list-disc space-y-0.5 pl-4 text-muted-foreground">
      <li>Takes only the <strong class="text-foreground">top K</strong> (default 20) of the already-fused list.</li>
      <li>A cross-encoder re-reads each of those transcripts against your combined text
        (<code>keyword + meaning</code>) and reorders just that head.</li>
      <li>Everything below K keeps its first-stage order — no second search is run.</li>
      <li><strong class="text-foreground">Text-only:</strong> it never looks at the image or the vectors.
        For image-only search there is no query text, so rerank is a no-op.</li>
    </ul>
  </div>
  ...
  <div class="flex flex-col gap-1.5">
    ...
    <div class="overflow-hidden rounded border border-border text-[10px]">
      <div class="grid grid-cols-[1fr_1.6fr] text-muted-foreground">
        ...
        <div class="border-b border-l border-border p-1.5">How many chunks to show (N, default 100).</div>
      </div>
      <div class="grid grid-cols-[1fr_1.6fr] text-muted-foreground">
        <div class="border-b border-border p-1.5 text-foreground">Rerank top</div>
        <div class="border-b border-l border-border p-1.5">Head size K the cross-encoder re-reads (default 20).</div>
      </div>
      <div class="grid grid-cols-[1fr_1.6fr] text-muted-foreground">
        <div class="border-b border-border p-1.5 text-foreground">Balance</div>
        <div class="border-b border-l border-border p-1.5">Keyword ↔ meaning weight. Hybrid only (ignored once an image is added).</div>
      </div>
      <div class="grid grid-cols-[1fr_1.6fr] text-muted-foreground">
        <div class="p-1.5 text-foreground">Match style</div>
        <div class="border-l border-border p-1.5">loose words, exact phrase, or fuzzy (typo-tolerant) keyword matching.</div>
      </div>
    </div>
  </div>
