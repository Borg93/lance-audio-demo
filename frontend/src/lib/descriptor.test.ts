...
const IMAGES = {
  id: 'images',
  tables: {
    items: { name: 'items', row_count: 5, version: 1, columns: [{ name: 'item_id', arrow_type: 'string', nullable: false }], indexes: [] },
  },
  declared: {
    identity: { key_fields: ['item_id'], doc_key: 'item_id', doc_key_pattern: '.*' },
    document: { table: 'items', media_blob: 'blob', mime: 'mime' },
    time: null,
    display: { title: ['label'], body: 'caption', caption: null, metadata: [{ field: 'label', label: 'Label' }] },
    search: { row_table: 'items', fts: { table: 'items', column: 'caption', language: 'English' }, vectors: {}, filterable: ['label'], rerank: false },
    atlas: [],
    capabilities: {},
  },
};
...
describe('DatasetView — transcripts corpus', () => {
  ...
  it('picks the first non-empty declared title field', () => {
    expect(v.title({ title_name: 'Press conf', audio_path: 'x.mp4', doc_id: 'abc' } as Row)).toBe('Press conf');
