<script lang="ts">
  ...
  const DECISION = [
    { when: 'You can quote the exact words or a name that was said', then: 'Keyword — type the phrase (Phrase for exact order, Fuzzy for typos). Turn on Rerank to float the closest wording up.' },
    { when: 'You only remember the gist, not the words', then: 'Vector — describe the idea in your own words; it matches meaning, so paraphrases and synonyms still land.' },
    { when: 'You remember what it LOOKED like, not what was said', then: "Scene — describe the picture (podium, placard, snowy street). It searches the AI's caption of each frame, not the pixels or the speech." },
    { when: 'You have a screenshot / photo from the clip', then: 'Image — attach it to find visual look-alikes. Look-alike only, not face / identity; add words if you also recall what was said.' },
    { when: 'You want exact words AND meaning', then: 'Hybrid — runs Keyword + Vector together; nudge the Balance slider toward keyword (precise) or meaning (looser).' },
    { when: "You're not sure which clue is strongest", then: 'Attach an image and add text → all-angles mode: up to four legs (Keyword, Vector, Scene, Image) vote equally; the dial is ignored.' },
  ];

  // Results troubleshooter: symptom → fix (fact-checked; postfilter can drop <N).
  const TROUBLESHOOT = [
    { symptom: 'A top hit has nothing to do with what is on screen', fix: 'It won on the transcript — someone SAID your words even if the frame shows something else. Use Scene if you meant what was SHOWN.' },
    { symptom: 'You attached an image but look-alikes are not dominating', fix: 'Expected: in mixed search the image is one equal vote (RRF), never a gate. Drop the text for Image-only, or add Scene to reinforce the look.' },
    { symptom: 'Fewer results came back than the count you set', fix: 'A Filter is dropping them — a postfilter trims after searching. Loosen/remove it, switch it to prefilter, or use Vector to widen recall.' },
    { symptom: 'Off-topic hits on top, good ones lower down', fix: 'Turn on Rerank to re-sort the top by transcript, push the Balance slider toward the signal you trust, or add a Filter.' },
    { symptom: 'Results feel noisy, scattered across press conferences', fix: 'Narrow with Filters (name, language, referenskod). With prefilter on they narrow the transcript legs (Keyword/Vector) before searching; the Image/Scene legs always filter after ranking.' },
    { symptom: 'Sure a chunk exists but nothing finds it', fix: 'Widen recall: add signals (Hybrid or Scene), raise the result count, clear Filters. Wrong exact words? Vector catches the meaning Keyword misses.' },
    { symptom: 'Rerank is on but the order barely changed', fix: 'Rerank reads transcripts only — a no-op for Image, and it ignores captions. For visual / scene intent, fix the query or signal instead.' },
  ];

  // Plain-language glossary (fact-checked rerank-scope wording).
  const GLOSSARY = [
    { term: 'Chunk', def: 'one flashcard — a few seconds of clip, with the spoken transcript on one side and the video frame (+ caption) on the other. The smallest thing search returns.' },
    { term: 'Caption', def: 'a short Swedish sentence an AI wrote about what is VISIBLE in the frame. Lets you find things shown but never spoken.' },
    { term: 'Keyword (BM25)', def: 'the literal-minded librarian — finds the exact words you typed (with phrases + small typo-tolerance) in the transcript. Great for names/quotes, blind to synonyms.' },
    { term: 'Embedding / Vector', def: "turns text into a dot on a giant 'meaning map' so similar ideas land near each other. Vector search returns transcripts whose dot sits closest to your query, even with no shared words." },
    { term: 'Cosine', def: 'the ruler for "how close" two dots point on the meaning map. Same direction = alike. Just the yardstick, not a setting.' },
    { term: 'RRF', def: 'the fair vote-counter — rewards results that rank HIGH on MORE lists, using ranks not raw scores. (The Balance slider is the exception: it blends raw scores, Hybrid only.)' },
    { term: 'Cross-encoder / Rerank', def: "the careful second reader — re-reads each candidate's transcript against your text and reshuffles the head. It scores by transcript only, so it can't use the image or caption signal — but it still re-ranks whatever candidates sit in the head." },
    { term: 'Prefilter vs Postfilter', def: 'Filters are a guest list. Prefilter checks IDs at the door (search only looks inside allowed chunks); Postfilter lets everyone search then removes the uninvited after — so you may get fewer results than you asked for.' },
    { term: 'Scene', def: 'searching the PICTURE side with text — compares your words to the frame captions, surfacing what is on screen even when unspoken.' },
    { term: 'Image', def: 'drag in a picture to find visual look-alikes (similar frames). Matches appearance, not who a person is.' },
  ];

  // Data-flow: the journey of a query end-to-end (from backend/search/service.py).
  const JOURNEY = [
    { label: 'You ask', detail: 'your text and/or attached image (plus any Filters) go to the backend.' },
    { label: 'Words & pictures → vectors', detail: 'the embedding service (a vLLM model) turns your text into a meaning-vector and your image into an image-vector. Skipped for pure Keyword and Scene-keyword — those are word-matching, no vectors.' },
    { label: 'Each signal queries a table', detail: 'Keyword & Vector read the chunks table (transcripts); Image & Scene read the chunk_frames table (one row per video frame), then join each frame back to its chunk.' },
    { label: 'Merge', detail: "the backend fuses the per-signal ranked lists — RRF, or Lance's native hybrid / the Balance slider." },
    { label: 'Optional rerank', detail: 'the reranker service (a second vLLM model) re-reads the top transcripts and reorders just the head.' },
    { label: 'Dress the results', detail: "the backend attaches each hit's caption and word-level alignments." },
    { label: 'Back to you', detail: 'one uniform result shape for every mode; click a hit and the player plays it with the transcript synced.' },
  ];

  // Scenario matrix: the dial × image → mode/passes mapping, straight from
  // buildSpec (search-bar.svelte) + run_search (service.py).
  const SCENARIOS = [
    { give: 'Keyword — text, no image', mode: 'fts', passes: 'Keyword (BM25)', combined: 'single list', slider: '—', rerank: 'yes' },
    { give: 'Vector — text, no image', mode: 'semantic', passes: 'Vector (text embedding)', combined: 'single list', slider: '—', rerank: 'yes' },
    { give: 'Scene — text, no image', mode: 'scene', passes: 'Scene caption (meaning, or BM25 via the Scene toggle)', combined: 'single list', slider: '—', rerank: 'yes' },
    { give: 'Hybrid — text, no image', mode: 'hybrid', passes: 'Keyword + Vector', combined: 'RRF, or the Balance slider', slider: 'available', rerank: 'yes' },
    { give: 'Image only — no text', mode: 'visual', passes: 'Image (frame)', combined: 'single list', slider: '—', rerank: 'no-op (no text)' },
    { give: 'Keyword + image + text', mode: 'all', passes: 'Keyword + Vector + Image + Scene', combined: 'equal-weight RRF', slider: 'ignored', rerank: 'yes', highlight: true },
    { give: 'Vector + image + text', mode: 'all', passes: 'Keyword + Vector + Image + Scene', combined: 'equal-weight RRF', slider: 'ignored', rerank: 'yes', highlight: true },
    { give: 'Scene + image + text', mode: 'all', passes: 'Keyword + Vector + Image + Scene', combined: 'equal-weight RRF', slider: 'ignored', rerank: 'yes', highlight: true },
    { give: 'Hybrid + image + text', mode: 'all', passes: 'Keyword + Vector + Image + Scene', combined: 'equal-weight RRF', slider: 'ignored', rerank: 'yes', highlight: true },
  ];
  ...
  const demoMode = $derived.by(() => {
    ...
    return dial === 'keyword' ? 'fts' : dial === 'vector' ? 'semantic' : dial === 'scene' ? 'scene' : 'hybrid';
  });
  const demoLegs = $derived.by((): string[] => {
    switch (demoMode) {
      case 'all': return ['keyword', 'vector', 'image', 'scene'];
      case 'hybrid': return ['keyword', 'vector'];
      case 'fts': return ['keyword'];
  ...
  const DEMO_LANES = [
    { key: 'keyword', name: 'Keyword', icon: '⌨️', side: 'said', on: 'border-sky-400 bg-sky-400/10 text-sky-600 dark:text-sky-300' },
    { key: 'vector', name: 'Vector', icon: '💬', side: 'said', on: 'border-primary bg-primary/10 text-primary' },
    { key: 'image', name: 'Image', icon: '🖼️', side: 'shown', on: 'border-amber-400 bg-amber-400/10 text-amber-600 dark:text-amber-300' },
    { key: 'scene', name: 'Scene', icon: '🎬', side: 'shown', on: 'border-emerald-400 bg-emerald-400/10 text-emerald-600 dark:text-emerald-300' },
  ];
  ...
  const WORKED = [
    {
      ...
      steps: [
        { stage: 'Filters (none set)', yields: 'No filter → both legs search the whole corpus, nothing narrowed away.' },
        { stage: 'Keyword leg — BM25 on transcript', yields: "Matches only literal spoken words of your query 'hjälpa hushåll med elräkningar'. #1 A 'hjälpa hushåll som inte klarar sina elräkningar'; #2 E 'hushållens elräkningar har tredubblats'. The paraphrase C is ABSENT — it never says your words, so BM25 scores it 0." },
        { stage: 'Vector leg — cosine on text_embedding', yields: "Matches meaning, ignores shared words. #1 C 'vi vill stötta hushållen med elkostnaderna'; #2 A. C wins here despite zero word overlap with your query." },
        { stage: 'RRF fuse — rank-based, equal weight', yields: 'A is on BOTH lists (Keyword #1, Vector #2) → highest combined score. C is on ONE list but at #1 → just behind. Fused order: A, C, then the rest. A leads only because it scored on both legs.' },
        { stage: 'Rerank top-K — cross-encoder, transcript-only', yields: "Re-reads each candidate's transcript against your text. Recognises 'stötta hushållen med elkostnaderna' as the closest paraphrase and floats C to #1, A #2. (Ignores frames/captions.)" },
      ],
    ...
    },
    {
      ...
      steps: [
        { stage: 'Pick the mode', yields: "Type the picture in words: 'snöig gata utomhus', dial = Scene (no image attached, so not 'all'). Backend mode = scene." },
        { stage: 'Why NOT Keyword', yields: "Keyword runs BM25 on the TRANSCRIPT only. This clip's transcript is energy talk ('elpriserna har stigit...') — 'snö' / 'gata' are never spoken, so BM25 scores it 0. It would instead surface a clip that literally says 'gata' (a street-repair debate) — the wrong scene." },
        { stage: 'Why NOT Vector', yields: "Vector embeds the TRANSCRIPT and compares to your query. The transcript is about elpriser — far from 'snöig gata' in meaning space — so the clip ranks ~#80, effectively missed. Vector reads what was SAID, never what is shown." },
        { stage: 'Scene leg — the one pass', yields: "Your text → one vector, compared by cosine to every frame's caption_embedding, then joined back to its chunk. Top captions: #1 .81 'minister intervjuas på en snöig gata utomhus'; #2 .77 'reporter på en snötäckt trottoar'; #3 .69 'vinterväg med bilar, snö faller'." },
        { stage: 'Fuse — nothing to fuse', yields: 'Scene is a single leg: no RRF, no Balance slider. The cosine-ranked caption list IS the result.' },
        { stage: 'Rerank — would it help? No', yields: "Rerank re-reads only each candidate's TRANSCRIPT vs your text. 'snöig gata' has no match in the energy transcript, so it would score the right clip LOW and could push it down — and it never reads the caption that won. Leave it off for visual intent." },
      ],
    ...
    },
    {
      ...
      steps: [
        { stage: 'Build the spec — dial IGNORED', yields: "You drag in podium.png and type 'exportstopp'. Image + text → mode = all no matter where the dial sits; the Balance slider is dropped. Your text drives the Keyword + Vector + Scene legs; the image drives the Image leg. Each leg over-fetches ~3×N candidates." },
        { stage: 'Leg 1 — Keyword (BM25 on transcript)', yields: "Literal 'exportstopp': #1 chunk_842 'vi inför ett exportstopp för...'; #2 chunk_119 'exportstoppet träder i kraft'. Even though you 'meant' Vector by habit, your text still ran a real BM25 keyword leg." },
        { stage: 'Leg 2 — Vector (meaning on text_embedding)', yields: "Paraphrases with no shared word: #1 chunk_507 'stoppa utförseln av krigsmateriel'; #2 chunk_842. chunk_842 ranks well here too." },
        { stage: 'Leg 3 — Image (frame_embedding vs your photo)', yields: 'Pure look-alike, no text: #1 chunk_077 (near-identical mic cluster); #2 chunk_842 (same podium room); #3 chunk_415. It found visually-similar podiums; it does NOT know who is standing there.' },
        { stage: 'Leg 4 — Scene (caption_embedding vs your text)', yields: "Your text vs each frame's caption: #1 chunk_842 'minister vid talarstol, plakat om vapenexport'; #2 chunk_203 'diagram över exportsiffror'. Your text drove a second picture-side leg." },
        { stage: 'RRF fuse — equal weight, rank-based', yields: 'chunk_842 appears near the top of ALL FOUR lists → highest combined score. chunk_077 is in 2 lists, chunk_507 in 2. Nobody is the unanimous #1, but chunk_842 is high on the MOST lists → it wins. The image was one vote of four, never a gate.' },
        { stage: 'Rerank the head — text-only, top-K', yields: "Reads ONLY transcripts vs 'exportstopp'. chunk_842 literally says 'vi inför ett exportstopp' → stays #1. A look-alike like chunk_415 (right podium, transcript about elpriser) gets pushed DOWN — rerank can't see its frame, only its off-topic words." },
      ],
    ...
    },
    {
      ...
      steps: [
        { stage: 'Setup', yields: "Mode = Hybrid. Query 'elstöd till hushållen'. Count N = 20. Filter namn LIKE '%Andersson%'. Filter toggle = POSTFILTER. Hybrid runs Keyword + Vector over the whole corpus, then fuses by RRF." },
        { stage: 'Postfilter = rank everyone, then drop', yields: 'Each leg ranks the WHOLE corpus (mostly other ministers) and keeps its candidate pool, THEN deletes the non-Andersson rows. Andersson is only a fraction of each pool, so few survive — the fused list comes back well short of 20.' },
        { stage: 'The vanished clip', yields: "A clip you remember ('elstödet kommer redan i januari', Andersson) ranks low corpus-wide — it competes against every minister — so it falls outside each leg's fetched pool and is dropped before you ever see it. That's your missing good one." },
        { stage: 'Diagnosis', yields: 'Postfilter applies the Filter AFTER each leg ranks the full corpus, so your count (20) is the size BEFORE the drop, not what is delivered — and anything ranked low across the whole corpus is invisible.' },
        { stage: 'Fix — flip to PREFILTER', yields: 'Set the Filter to prefilter (the default; the backend passes prefilter=True). Now the Filter narrows the corpus to ONLY Andersson\'s chunks BEFORE either leg ranks.' },
        { stage: 'Result after prefilter', yields: "Each leg now ranks within Andersson only, so it returns a full 20 — and 'elstödet kommer redan i januari' competes only against other Andersson clips, where it ranks near the top of both legs, so RRF puts it high." },
      ],
  ...
  <div class="mx-auto max-w-3xl px-6 py-8">
    ...
    {#if tab === 'search'}
      ...
      <section use:reveal class="reveal mb-10">
        <h2 class="text-foreground mb-1 text-lg font-semibold">Two worlds: was it said, or was it shown?</h2>
        <p class="text-muted-foreground mb-4 text-sm leading-relaxed">
          ...
          <strong class="text-foreground">shown</strong> (the frame + its Swedish caption). Each signal reads one
          side, so figure out which side your memory lives on and the right tool almost picks itself.
        </p>
        <div class="grid gap-3 sm:grid-cols-2">
          <div use:reveal class="reveal rounded-lg border border-sky-400/40 bg-sky-400/5 p-3">
            ...
            <p class="text-muted-foreground mt-1 text-xs leading-relaxed">
              What someone spoke. Read by <span class="font-medium text-sky-500 dark:text-sky-300">Keyword</span>
          ...
          <div use:reveal={{ delay: 90 }} class="reveal rounded-lg border border-emerald-400/40 bg-emerald-400/5 p-3">
            <div class="text-foreground font-medium">👁️ Shown — the frame</div>
            <p class="text-muted-foreground mt-1 text-xs leading-relaxed">
              What was on screen. Read by
              <span class="font-medium text-emerald-500 dark:text-emerald-300">Scene</span> (describe it in words →
              matches the caption) and <span class="font-medium text-amber-500 dark:text-amber-300">Image</span>
              (hand it a photo → visual look-alikes).
            </p>
          </div>
        </div>
        <p class="text-muted-foreground mt-3 text-sm leading-relaxed">
          Not sure which world? Attach an image and add words — that opens the all-angles mode, which votes across
          both worlds at once (the image is one vote, never the boss). And note: <strong class="text-foreground">Rerank</strong>
          only re-reads the <em>said</em> side, and <strong class="text-foreground">Filters</strong> aren't a world —
          they narrow the room <em>before</em> (or, as a postfilter, after) every search.
        </p>
      </section>
      ...
      <section use:reveal class="reveal mb-10">
        ...
        <p class="text-muted-foreground mb-3 text-sm leading-relaxed">
          Picture a vast map where every idea gets a pin, and similar meanings land near each other — "the minister
          stepped down" sits a short walk from "the minister resigned", even with no shared word. An
          <strong class="text-foreground">embedding</strong> is just that pin. Vector search drops your query on the
          same map and hands back the nearest chunks — so wording barely matters.
        </p>
        <div class="border-border bg-surface2 rounded-lg border p-3 text-xs leading-relaxed">
          <ul class="text-muted-foreground space-y-1.5">
            <li><strong class="text-foreground">Embedding = a map pin for meaning.</strong> Close = similar; far = unrelated. That distance is the search.</li>
            <li><strong class="text-foreground">"Cosine" = pointing the same direction.</strong> Two ideas headed the same way are a match — length / loudness doesn't matter.</li>
            <li><strong class="text-foreground">One shared map for text AND captions.</strong> Transcript-meanings and caption-meanings live in the same space, so typed words can find a frame's caption — that's the trick behind Scene.</li>
            <li><strong class="text-foreground">Meaning, not letters.</strong> Vector shrugs off spelling but can drift on names / codes / quotes — that's where Keyword (or Filters) earns its keep.</li>
          </ul>
        </div>
      </section>
      ...
      <section use:reveal class="reveal mb-10">
        <h2 class="text-foreground mb-1 text-lg font-semibold">Why cast several nets, then pull them together</h2>
        <p class="text-muted-foreground mb-3 text-sm leading-relaxed">
          No single signal sees the whole moment — Keyword and Vector only hear the words; Scene and Image only see
          the picture. So the app asks several witnesses at once — a stenographer, a paraphraser, a set photographer —
          and trusts the clips they <em>agree</em> on. Agreement across independent signals is the strongest hint a
          clip is really relevant.
        </p>
        <div class="border-border bg-surface2 rounded-lg border p-3 text-xs leading-relaxed">
          <ul class="text-muted-foreground space-y-1.5">
            <li><strong class="text-foreground">Wide net first, tighten later.</strong> In the fused <code class="font-mono">all</code> mode each leg over-fetches ~3× more candidates than you asked for (recall — don't miss it), then fusion + Rerank squeeze to the best few (precision).</li>
            <li><strong class="text-foreground">Agreement is the relevance signal.</strong> Appearing once is a maybe; ranking high on three lists is a strong yes.</li>
            <li><strong class="text-foreground">Votes, not vetoes.</strong> In image-plus-text search every leg is one equal vote (RRF); a missing leg just abstains and the rest decide.</li>
            <li><strong class="text-foreground">Catches synonyms AND silence.</strong> Vector saves you when the words differ; Scene saves you when the thing on screen was never spoken.</li>
          </ul>
        </div>
      </section>
          ...
          <li use:reveal class="reveal flex gap-3">
            ...
            <div>
              ...
              <p class="text-muted-foreground mt-0.5 text-xs leading-relaxed">
                ...
                <code class="font-mono">WHERE</code>. For the <strong class="text-foreground">transcript legs</strong>
                (Keyword, Vector, Hybrid), <em>prefilter</em> narrows the corpus <em>before</em> they search (toggle
                off → after); the <strong class="text-foreground">frame legs</strong> (Image, Scene) always apply it
          ...
          <li use:reveal={{ delay: 70 }} class="reveal flex gap-3">
            ...
            <div>
              ...
              <p class="text-muted-foreground mt-0.5 text-xs leading-relaxed">
                ...
                the same time</strong>. No leg goes first and none outranks another. In <code class="font-mono">all</code>
        ...
        <div class="grid gap-3 sm:grid-cols-2">
          {#each SIGNAL_EXAMPLES as s, i (s.name)}
            ...
            >
              ...
              <div class="mb-2">
                <code class="bg-card text-primary rounded px-1.5 py-0.5 font-mono text-[11px]">{s.q}</code>
              </div>
              <p class="text-muted-foreground">
                <span class="font-medium text-emerald-600 dark:text-emerald-300">Finds:</span> {s.finds}
              </p>
              <p class="text-muted-foreground mt-1">
                <span class="font-medium text-amber-600 dark:text-amber-300">Misses:</span> {s.misses}
              </p>
              <p class="text-muted-foreground mt-1">
                <span class="text-foreground font-medium">Use when:</span> {s.use}
              </p>
            </div>
          {/each}
        </div>
      ...
      <section use:reveal class="reveal mb-10">
        ...
        <div class="border-border overflow-hidden rounded-lg border text-xs">
          {#each DECISION as r, i (r.when)}
            <div use:reveal={{ delay: i * 60 }} class="reveal grid grid-cols-[1fr_1.4fr] last:[&>div]:border-b-0">
      ...
      <!-- 5b · scenario matrix -->
      <section use:reveal class="reveal mb-10">
        <h2 class="text-foreground mb-1 text-lg font-semibold">Scenario matrix: exactly what runs, and when</h2>
        <p class="text-muted-foreground mb-3 text-sm leading-relaxed">
          ...
          <strong class="text-foreground">(2)</strong> attaching an image is the master switch — image + any text
          becomes the 4-way <code class="font-mono">all</code> (dial &amp; slider ignored); image alone becomes
          <code class="font-mono">visual</code>. Inside any multi-pass mode
          <strong class="text-foreground">no leg is prioritised</strong> — fusion is equal-weight RRF (Hybrid may
          instead use the Balance slider).
        </p>
        ...
        <div class="bg-card border-border mb-3 rounded-xl border p-4">
          <div class="flex flex-wrap items-center gap-1.5">
            ...
            {#each DEMO_DIALS as d (d)}
              <button
                type="button"
                onclick={() => (dial = d)}
                class="rounded-md border px-2 py-1 text-xs font-medium capitalize transition-all {dial === d
                  ? 'border-primary bg-primary/10 text-primary'
                  : 'border-border text-muted-foreground hover:text-foreground'} {hasImage ? 'opacity-40' : ''}"
              >{d}</button>
            {/each}
          ...
          <div class="mt-3 flex flex-wrap items-center gap-2 text-xs">
            ...
            {#if demoMode}
              <code class="bg-primary/15 text-primary rounded px-2 py-0.5 font-mono font-semibold">{demoMode}</code>
              <span class="text-muted-foreground">{demoLegs.length} pass{demoLegs.length > 1 ? 'es' : ''}</span>
            {:else}
          ...
          <div class="mt-3 flex flex-wrap items-center gap-1.5 text-[11px]">
            ...
            >{demoLegs.length > 1 ? (demoMode === 'hybrid' ? 'fused: RRF / slider' : 'fused: equal RRF') : 'no fusion'}</span>
            <span
          ...
          <p class="text-muted-foreground mt-3 text-xs leading-relaxed">
            {#if hasImage && hasText}
              Image + text → the dial is <strong class="text-foreground">ignored</strong> and all four passes vote
              equally. Even with Dial = Vector, your text <em>still</em> runs the Keyword/BM25 leg — the image just
              adds a fourth voice (never a gate).
            {:else if hasImage}
              Image alone → a single <code class="font-mono">visual</code> pass; Rerank is a no-op without text.
            {:else if demoMode}
              No image → your dial picks the mode. Toggle <strong class="text-foreground">🖼️ Image</strong> and
              watch it collapse to the 4-way <code class="font-mono">all</code>.
            {:else}
        ...
        <div class="text-muted-foreground mb-1.5 text-[11px] font-medium tracking-wide uppercase">Full reference</div>
            ...
            <tbody>
              {#each SCENARIOS as s (s.give)}
                <tr class="text-muted-foreground {s.highlight ? 'bg-highlight/10' : ''}">
                  <td class="border-border text-foreground border-b p-2 font-medium whitespace-nowrap">{s.give}</td>
                  <td class="border-border border-b border-l p-2"><code class="text-primary font-mono">{s.mode}</code></td>
        ...
        <div
          ...
          class="reveal border-amber-500/40 bg-amber-500/5 text-muted-foreground mt-3 rounded-lg border p-3 text-sm leading-relaxed"
        >
          <strong class="text-foreground">What adding an image does:</strong> Keyword, Vector, Scene, <em>or</em>
          Hybrid all collapse to the same 4-way <code class="font-mono">all</code> the moment you attach one. So
          <strong class="text-foreground">Vector + image is not "just the meaning &amp; image legs"</strong> —
          <code class="font-mono">all</code> runs <em>four equal legs</em>, so your text <em>also</em> drives a
          literal Keyword/BM25 leg and a Scene/caption leg. The dial you picked and the Balance slider stop
          mattering; the image is simply added as a fourth equal voice (never a gate). Image with no text stays a
          single <code class="font-mono">visual</code> pass.
        </div>
      </section>

      <!-- 5c · interactive worked examples -->
      <section use:reveal class="reveal mb-10">
        <h2 class="text-foreground mb-1 text-lg font-semibold">Worked examples: watch a full search run</h2>
        <p class="text-muted-foreground mb-3 text-sm leading-relaxed">
        ...
        >
          <span class="font-medium text-emerald-600 dark:text-emerald-300">✓ Verified live</span> against the
          running backend: Keyword, Vector, Hybrid, rerank, the Balance slider, Image (a posted frame self-matched
          at&nbsp;#1) and <code class="font-mono">all</code> all return real results; a name filter returned
          <strong class="text-foreground">15</strong> hits with prefilter vs <strong class="text-foreground">0</strong>
          with postfilter; fuzziness&nbsp;2 recovered a typo that fuzziness&nbsp;0 missed. The chunk IDs and cosines
          in the examples below are illustrative — the <em>mechanics</em> they show are what's verified.
        </div>
        <div class="mb-3 flex flex-wrap gap-1.5">
          {#each WORKED as w, i (w.key)}
            <button
              type="button"
              onclick={() => (exIdx = i)}
              class="rounded-md border px-2.5 py-1.5 text-xs font-medium transition-colors {exIdx === i
                ? 'border-primary bg-primary/10 text-primary'
        ...
        {#key exIdx}
          ...
          <div class="bg-card border-border rounded-xl border p-4">
            <div class="flex flex-wrap items-center gap-2">
              ...
              <code class="bg-primary/15 text-primary rounded px-2 py-0.5 font-mono text-[11px] font-semibold">mode: {w.mode}</code>
            </div>
            ...
            <ol class="mt-4 space-y-3">
              {#each w.steps as s, i (s.stage)}
                <li use:reveal={{ delay: i * 70 }} class="reveal flex gap-3">
                  <span class="bg-primary/15 text-primary grid size-6 shrink-0 place-items-center rounded-full text-xs font-semibold">{i + 1}</span>
                  <div class="min-w-0">
                    <div class="text-foreground text-xs font-semibold">{s.stage}</div>
                    <p class="text-muted-foreground mt-0.5 text-xs leading-relaxed [overflow-wrap:anywhere]">{s.yields}</p>
                  </div>
                </li>
              {/each}
            </ol>

            <div use:reveal class="reveal border-primary/40 bg-primary/5 mt-4 rounded-lg border p-3 text-xs leading-relaxed">
              <span class="text-foreground font-semibold">Result:</span>
              <span class="text-muted-foreground">{w.result}</span>
            </div>
            <div use:reveal class="reveal bg-surface2 border-border mt-2 rounded-lg border p-3 text-xs leading-relaxed">
      ...
      <section use:reveal class="reveal mb-10">
        ...
        <p class="text-muted-foreground mb-3 text-sm leading-relaxed">
          Think of every active signal as a judge handing in a ranked shortlist. The clip that lands high on the
          <strong class="text-foreground">most</strong> shortlists wins — even if it's nobody's outright #1. Here
          it is with two judges on 5 clips (A–E), each returning a ranked list:
        </p>
        <div class="grid gap-3 sm:grid-cols-2">
        ...
        <div
          use:reveal
          class="reveal border-border bg-card mt-3 rounded-lg border p-3 text-xs leading-relaxed text-muted-foreground"
        >
          <div class="text-foreground mb-1 font-medium">How the merge actually works (and why any number of legs is fine)</div>
          The legs run <strong class="text-foreground">independently</strong> — none feeds its hits to another
          (the image's results are <em>not</em> handed to the Vector leg). Each returns its <em>own</em> ranked list
          over the whole library. RRF then pours all the lists into <strong class="text-foreground">one pot keyed by
          chunk</strong>: a chunk's score is the <strong class="text-foreground">sum</strong> of
          <code class="font-mono">1/(60 + rank)</code> from <em>every</em> list it appears in (absent from a list →
          adds 0). So it's a <strong class="text-foreground">union with added scores</strong> — not a chain, and not a
          plain concatenation. Because it's just a sum, it extends to <strong class="text-foreground">any number of
          lists</strong> (2 for Hybrid, up to 4 for <code class="font-mono">all</code>) — you add one more term per
          leg. A chunk only one leg found still shows up (it just scores low); a chunk several legs rank highly wins.
        </div>
      </section>
      ...
      <section use:reveal class="reveal mb-10">
        ...
        >
          ...
          <code class="bg-card text-primary rounded px-1 font-mono">elcertifikat</code> → slide toward
          <strong class="text-foreground">Keyword</strong> for that exact word; searching
          <code class="bg-card text-primary rounded px-1 font-mono">vad regeringen gör åt höga elpriser</code> →
          slide toward <strong class="text-foreground">Vector</strong> to catch a clip that says "stötta hushållen
          med elkostnaderna". The slider only re-weights the two <em>text</em> passes — a weighted blend of their
          scores, not rank-voting — and does nothing for Scene or Image.
        </div>
        <div class="grid gap-3 sm:grid-cols-2">
          <div use:reveal class="reveal bg-surface2 border-border rounded-lg border p-3 text-xs">
      ...
      <section use:reveal class="reveal bg-card border-border mb-10 rounded-lg border p-4">
        ...
        >
          ...
          <code class="bg-card text-primary rounded px-1 font-mono">höjda räntor</code>: the fused top-3 might be
          ① a "minister vid podium" clip that drifted in on Vector, ② a stakeout that says "räntan" once, ③ the
          clip that actually says "vi måste hantera de höjda räntorna". The cross-encoder reads only each
          transcript and reorders to <strong class="text-foreground">③ ① ②</strong> — promoting the clip whose
          spoken words really match.
        </div>
      </section>
      ...
      <section use:reveal class="reveal border-border bg-surface2 mb-10 rounded-lg border p-4">
        ...
        <p class="text-muted-foreground text-sm leading-relaxed">
          ...
          SQL) is the only thing that narrows the library — and it has nothing to do with the
          image. A filter becomes a <code class="font-mono">WHERE</code> clause. For the
          <strong class="text-foreground">transcript legs</strong> (Keyword, Vector, Hybrid), <em>prefilter</em>
          narrows the corpus before they search (off → after); the <strong class="text-foreground">frame legs</strong>
          (Image, Scene) always filter after ranking. So <strong class="text-foreground">Filter = a fence around the whole
        ...
        <div use:reveal class="reveal border-border mt-3 overflow-hidden rounded-lg border text-xs">
          <div class="bg-surface2 text-foreground grid grid-cols-[1fr_1.5fr] font-medium">
            ...
            <div class="border-border border-b border-l p-2">What it targets (all on the chunk's metadata)</div>
          </div>
          <div class="text-muted-foreground grid grid-cols-[1fr_1.5fr]">
            ...
            <div class="border-border border-b border-l p-2"><code class="font-mono">language = …</code> — exact match</div>
          </div>
          <div class="text-muted-foreground grid grid-cols-[1fr_1.5fr]">
            <div class="border-border text-foreground border-b p-2">Name (namn)</div>
            <div class="border-border border-b border-l p-2"><code class="font-mono">namn LIKE '%…%'</code> — substring match</div>
          </div>
          <div class="text-muted-foreground grid grid-cols-[1fr_1.5fr]">
            <div class="border-border text-foreground border-b p-2">Ref (referenskod)</div>
            <div class="border-border border-b border-l p-2"><code class="font-mono">referenskod LIKE '%…%'</code> — substring match</div>
          </div>
          <div class="text-muted-foreground grid grid-cols-[1fr_1.5fr]">
            <div class="border-border text-foreground border-b p-2">Internal ID (extraid)</div>
            <div class="border-border border-b border-l p-2"><code class="font-mono">extraid = …</code> — exact match</div>
          </div>
          <div class="text-muted-foreground grid grid-cols-[1fr_1.5fr]">
            <div class="border-border text-foreground p-2">Raw SQL</div>
            <div class="border-border border-l p-2">any expression over chunk columns, ANDed in verbatim</div>
          </div>
        </div>
        <p use:reveal class="reveal text-muted-foreground mt-2 text-xs leading-relaxed">
          ...
          <code class="font-mono">chunks</code> table's metadata — <em>not</em> frame attributes — so even Image and
          Scene results are filtered by <em>their chunk's</em> language / name / ref / id.
        </p>
        ...
        >
          ...
          <code class="bg-surface2 text-primary rounded px-1 font-mono">nya regler</code> with a filter
          <code class="bg-surface2 rounded px-1 font-mono">namn = "Andersson"</code> and N = 10.
          <strong class="text-foreground">Prefilter (on):</strong> every leg that runs (here Keyword + Vector)
          searches <em>only</em> Andersson's chunks → a full 10 ranked Andersson hits.
          <strong class="text-foreground">Postfilter (off):</strong> each leg first finds its top 10 across
          <em>all</em> ministers, then drops the non-Andersson ones — if only 3 were Andersson, you see 3. A tight
          postfilter can come up short.
        </div>
      </section>
      ...
      <section use:reveal class="reveal mb-10">
        ...
        <p class="text-muted-foreground mb-4 text-sm leading-relaxed">
          Every hit won a popularity contest among the signals you turned on. Read a surprising result by asking
          "which side did it win on?", and fix a bad list by changing <em>who votes</em>, not by staring harder.
          Mental dial: <strong class="text-foreground">recall vs precision</strong> — widen when you're missing
          things, tighten when the top is junk.
        </p>
        <div class="border-border overflow-hidden rounded-lg border text-xs">
          {#each TROUBLESHOOT as r, i (r.symptom)}
            <div use:reveal={{ delay: i * 50 }} class="reveal grid grid-cols-[1fr_1.5fr] last:[&>div]:border-b-0">
      ...
      <section use:reveal class="reveal mb-10">
        ...
        <ol class="space-y-3">
          {#each JOURNEY as s, i (s.label)}
            <li use:reveal={{ delay: i * 60 }} class="reveal flex gap-3">
              <span class="bg-primary text-primary-foreground grid size-6 shrink-0 place-items-center rounded-full text-xs font-semibold">{i + 1}</span>
              <div>
        ...
        <div class="mt-4 grid gap-3 sm:grid-cols-2">
          <div use:reveal class="reveal bg-surface2 border-border rounded-lg border p-3 text-xs leading-relaxed">
            <div class="text-foreground font-medium">Two tables, one key</div>
            <p class="text-muted-foreground mt-1">
              <code class="font-mono">chunks</code> holds the transcript side; <code class="font-mono">chunk_frames</code>
              holds one row per video frame (its caption + image vector). They're linked by
              <code class="font-mono">(doc_id, speech_id, chunk_id)</code>. Image / Scene search the frames table,
              then join back — which is why every result is a <em>chunk</em>, even when the match was visual.
            </p>
          </div>
          <div use:reveal={{ delay: 90 }} class="reveal bg-surface2 border-border rounded-lg border p-3 text-xs leading-relaxed">
            <div class="text-foreground font-medium">One shared embedding space</div>
            <p class="text-muted-foreground mt-1">
              Transcript and caption vectors live in the <em>same</em> space, so a single query-text vector can be
              compared to both (Vector → transcripts, Scene → captions). Your image vector lives with the frame
              vectors (Image). Same map, different pins.
            </p>
          </div>
          <div use:reveal={{ delay: 180 }} class="reveal bg-surface2 border-border rounded-lg border p-3 text-xs leading-relaxed sm:col-span-2">
            ...
            <p class="text-muted-foreground mt-1">
              ...
              <strong class="text-foreground">backend</strong>, which talks to two separate model servers — one for
              <strong class="text-foreground">embeddings</strong>, one for
              <strong class="text-foreground">reranking</strong> — and reads the LanceDB tables on disk. If the
              embedding server is down a vector search fails fast with a clear error; if an embedding column was never
              built, that leg returns nothing in a fused search and the others carry on (a single Vector or Hybrid
              search instead shows a clear "not built" message). The
      ...
      <section use:reveal class="reveal mb-10">
        ...
        <dl class="border-border divide-border divide-y overflow-hidden rounded-lg border text-xs">
          {#each GLOSSARY as g, i (g.term)}
            <div use:reveal={{ delay: i * 40 }} class="reveal grid grid-cols-[8.5rem_1fr] gap-2 p-2">
