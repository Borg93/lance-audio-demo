<script lang="ts">
  ...
  const sizeOf = (mentions: number): number => 8 + Math.min(20, Math.sqrt(Math.max(1, mentions)) * 4);
  ...
  {#if detail?.entity}
    ...
    <aside class="border-border bg-card/30 w-80 shrink-0 overflow-auto border-l p-3">
      <div class="flex items-start justify-between gap-2">
        ...
        <button class="text-muted-foreground hover:text-foreground text-xs" onclick={() => (detail = null)}
          >✕</button
        >
      </div>
