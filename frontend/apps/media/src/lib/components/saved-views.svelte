...
<div class="relative">
  ...
  {#if open}
    <div
      class="absolute right-0 top-full z-20 mt-1 w-64 rounded-md border border-border bg-card p-2 shadow-md"
    >
      ...
      {#if views.length}
        <ul class="mt-2 flex flex-col gap-0.5">
          {#each views as v (v.name)}
            <li class="flex items-center gap-1 rounded hover:bg-muted/60">
              <button
      ...
      {:else}
        <p class="mt-2 px-1 text-[11px] text-muted-foreground">No saved views yet.</p>
      {/if}
    </div>
  {/if}
</div>
