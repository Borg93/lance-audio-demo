  ...
  {#each hits as h (hitKey(h))}
    ...
    >
      <button
        ...
        class="flex w-full gap-2 p-1.5 text-left transition-colors hover:bg-muted"
      >
      ...
      <div class="flex flex-wrap items-center gap-1 px-1.5 pb-1.5">
        {#each tags as tag (tag)}
          <span
            class="inline-flex items-center gap-1 rounded bg-primary/10 px-1.5 py-0.5 text-[9px] text-foreground"
          >
            {tag}
            <button
              type="button"
        ...
        {#if addingKey === key}
          ...
          <input
            class="w-24 rounded border border-border bg-background px-1 py-0.5 text-[9px] text-foreground outline-none focus:border-primary"
            placeholder="tag…"
            autofocus
            bind:value={draft}
        ...
        {:else}
          <button
            type="button"
            class="inline-flex items-center gap-0.5 rounded border border-dashed border-border px-1 py-0.5 text-[9px] text-muted-foreground hover:bg-muted hover:text-foreground"
            onclick={(e) => {
              e.stopPropagation();
              openAdd(key);
            }}
