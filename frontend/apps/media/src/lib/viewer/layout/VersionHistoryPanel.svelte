...
<section class="border-t border-border" data-testid="version-history">
  <button
  ...
  {#if !collapsed}
    <div class="flex flex-col gap-0.5 px-3 pb-3">
      ...
      {:else if versions.length === 0}
        <p class="text-xs text-muted-foreground">No history for this unit.</p>
      {:else}
        {#each versions as v, i (v.version)}
          <button
            class={cn(
              'flex items-center gap-2 rounded px-1 py-0.5 text-left hover:bg-muted/50',
              selected === v.version && 'bg-primary/10 ring-1 ring-primary/40',
            )}
          ...
          >
            ...
            <span class="text-[10px] tabular-nums text-muted-foreground" title="annotations at this version">
              {v.count}
            </span>
          </button>
