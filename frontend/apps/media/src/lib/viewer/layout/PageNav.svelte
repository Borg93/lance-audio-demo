...
<nav
  class="pointer-events-auto absolute bottom-2 left-1/2 z-10 flex -translate-x-1/2 items-center gap-1 rounded-lg border border-border bg-card/90 px-1 py-0.5 shadow-md backdrop-blur"
  data-testid="page-nav"
>
  <Button variant="ghost" size="icon-xs" title="Previous page" disabled={current <= 0} onclick={() => go(current - 1)}>
