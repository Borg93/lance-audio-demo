...
function fmtTime(s) {
  ...
  return h2 > 0 ? `${h2}:${pad(m)}:${pad(sec)}` : `${m}:${pad(sec)}`;
}
...
function queryTerms(q) {
  const stop = /* @__PURE__ */ new Set(["and", "or", "not", "near"]);
  return (q.match(/[\p{L}\p{N}_]+/gu) ?? []).map((t) => t.toLowerCase()).filter((t) => !stop.has(t));
}
const hitKey = (h2) => activeView().rowKey(h2);
...
export {
  ...
  fmtTime as G,
  HOME as H,
  makeHighlighter as I,
  queryTerms as J,
  getDocument as K,
  isElement as L,
  PresenceManager as M,
  isTouch as N,
  isTabbable as O,
  Portal as P,
  ...
  j as Y,
  n as Z,
  getFirstNonCommentChild as _,
  BoxSymbol as a,
  Focus_scope as a0,
  Escape_layer as a1,
  Dismissible_layer as a2,
  Text_selection_layer as a3,
  Scroll_lock as a4,
  TAB as a5,
  PAGE_UP as a6,
  PAGE_DOWN as a7,
  ...
  toReadonlyBox as t,
  mergeProps as u,
  isElementOrSVGElement as v,
  watch as w,
  ENTER as x,
  getDataChecked as y,
  boolToTrueOrUndef as z
};
