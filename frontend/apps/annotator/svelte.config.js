...
const config = {
  ...
  kit: {
    // Zone base: this app owns the /annotate path — pages AND dev/prod assets
    // live under it, so the viewer origin can proxy the whole prefix without
    // asset-path collisions (the micro-frontends skill's zones caveat).
    paths: { base: '/annotate' },
