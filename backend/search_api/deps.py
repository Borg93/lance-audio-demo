...
its own state deps); only :data:`~backend.deps.StateDep` is shared, since the
per-app :class:`~backend.state.AppState` is the one composition-level resource.
...
from backend.search_api.clients import ensure_embedder, ensure_reranker
